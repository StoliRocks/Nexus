...
```json
{
    ...
    "embedding": [0.0234, -0.0156, 0.0089, "...4096 floats..."],
...
| mappingKey | `{controlKey1}\|{controlKey2}` | `AWS.ControlCatalog#1.0#IAM.21\|NIST-SP-800-53#R5#AC-2` |
| reviewKey | `{reviewerId}#{timestamp}` | `expert-001#2024-01-15T14:30:00Z` |

**Note:** mappingKey uses `|` as delimiter and control keys are sorted alphabetically.
...
All tables have TTL enabled on the `expiryTime` attribute (or `ttl` for MappingJobs).
